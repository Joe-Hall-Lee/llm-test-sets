...
def visualize_and_correlate(file_path, model_columns):
    ...
    if not difficulty or not discrimination:
        ...
        print("\n难度最高的前十道题的序号（从 1 开始）:", [i + 1 for i in difficulty_top10])
        print("区分度最高的前十道题的序号（从 1 开始）:", [i + 1 for i in discrimination_top10])
    ...
    if len(ranked_indices) == len(reference_ranks) > 1:
        ...
        print("\n相关系数计算结果：")
        print(f"Spearman 相关系数: {spearman_corr:.4f} (p-value: {spearman_p:.4f})")
        print(
            f"Kendall Tau 相关系数: {kendall_corr:.4f} (p-value: {kendall_p:.4f})")
